# Переменные используются в экранах (screens.rpy) и стилях.
# ============================================================
...
## Значения 1920x1080 — это разрешение, в котором разрабатывалась игра.
init python:
    gui.init(1920, 1080)

## Включить проверку на недопустимые или нестабильные свойства в экранах или
## преобразованиях — помогает отлавливать ошибки оформления.
...
## Задаётся в шестнадцатеричном формате (#RRGGBB).
...
## Шрифт, используемый внутриигровым текстом (реплики, описания).
define gui.text_font = "DejaVuSans.ttf"

## Шрифт, используемый именами персонажей.
define gui.name_text_font = "DejaVuSans.ttf"

## Шрифт, используемый текстом вне игры (меню, кнопки, настройки).
define gui.interface_text_font = "DejaVuSans.ttf"

## Размер нормального текста диалога (в пикселях).
define gui.text_size = 33

## Размер имён персонажей.
define gui.name_text_size = 45

## Размер текста в пользовательском интерфейсе (кнопки, подписи).
define gui.interface_text_size = 33

## Размер заголовков в пользовательском интерфейсе (например, "Настройки").
define gui.label_text_size = 36

## Размер текста на экране уведомлений (всплывающие сообщения).
define gui.notify_text_size = 24

## Размер заголовка игры в главном меню.
...
## Фон главного меню — отдельное изображение.
define gui.main_menu_background = "images/bg/main_menu_bg.jpg"
## Фон игрового меню (при открытии Esc) — стандартный полупрозрачный оверлей.
...
## Высота текстового окна, содержащего диалог (в пикселях).
define gui.textbox_height = 278

## Местоположение текстового окна по вертикали экрана. 0.0 — верх, 0.5 — центр и
## 1.0 — низ (прижато к нижнему краю).
...
define gui.name_xpos = 360      # Отступ слева
define gui.name_ypos = 0        # Отступ сверху (0 — прямо над текстовым окном)
...
define gui.dialogue_xpos = 402   # Отступ слева внутри текстового окна
define gui.dialogue_ypos = 75    # Отступ сверху внутри текстового окна
...
## Кнопки-переключатели (радио-кнопки) — например, выбор режима окна.
define gui.radio_button_borders = Borders(27, 6, 6, 6)

## Кнопки-флажки (check-кнопки) — например, включение/отключение опций.
define gui.check_button_borders = Borders(27, 6, 6, 6)

## Кнопки подтверждения (в диалогах "Да"/"Нет") — выравнивание текста по центру.
define gui.confirm_button_text_xalign = 0.5

## Кнопки переключения страниц (в экране сохранения/загрузки).
define gui.page_button_borders = Borders(15, 6, 15, 6)

## Кнопки быстрого меню (в нижней части экрана).
...
## Кнопки Выбора ###############################################################
##
## Кнопки выбора используются во внутриигровых меню (варианты ответов).

define gui.choice_button_width = 1185
define gui.choice_button_height = None
define gui.choice_button_tile = False
define gui.choice_button_borders = Borders(150, 8, 150, 8)   # Большие отступы по бокам
define gui.choice_button_text_font = gui.text_font
define gui.choice_button_text_size = gui.text_size
define gui.choice_button_text_xalign = 0.5                  # Текст по центру
...
## Ширина и высота миниатюры, используемой слотом сохранения (скриншот).
define config.thumbnail_width = 384
define config.thumbnail_height = 216

## Количество колонок и рядов в таблице слотов (3×2 = 6 слотов на страницу).
...
## Вертикальная позиция индикатора пропуска (от верхнего края).
define gui.skip_ypos = 15

## Вертикальная позиция экрана уведомлений (от верхнего края).
...
## Позиция текста главного меню (выравнивание по правому краю).
...
## Генерируем рамки (отступы внутри рамок).
define gui.frame_borders = Borders(6, 6, 6, 6)

## Рамки, используемые в частях экрана подтверждения (большие отступы).
...
## Должны ли фоны рамок моститься? (False — масштабирование, True — тайлинг).
...
## Высота горизонтальных панелей, полос прокрутки и ползунков.
## Ширина вертикальных панелей, полос прокрутки и ползунков.
...
## Горизонтальные границы (отступы для изображений полос).
...
## Что делать с непрокручиваемыми полосами прокрутки в игровом меню.
## "hide" — скрывает их, None — показывает.
...
## Режим NVL ###################################################################
##
## Экран режима NVL показывает диалог NVL персонажей (текст на весь экран).
...
## Этот блок содержит настройки, которые применяются только на мобильных
## платформах (Android, iOS) для улучшения удобства использования.
...
init python:
    ...
    def small():
        ...
        ## Размеры шрифтов (увеличиваются для читаемости на маленьком экране).
        gui.text_size = 45
        ...
        ## Регулирует местоположение текстового окна (увеличивается высота,
        ## сдвигаются отступы).
        ...
        ## Местоположение кнопок слотов (2×2 вместо 3×2 для экономии места).
        gui.file_slot_cols = 2
        gui.file_slot_rows = 2

        ## Режим NVL (увеличенные размеры).
        gui.nvl_height = 255
